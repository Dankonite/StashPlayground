...
// Export maybeRenderAltImageA function
export function maybeRenderAltImageA(performerId: string): string | null {
  const { data } = GQL.useFindImagesQuery({
    variables: {
      image_filter: {
        ...
        tags: {
          modifier: GQL.CriterionModifier.Includes,
          value: ["7"], // insert your alt image tag id # here ex: ["1561"]
        },
      },
    },
  });
      ...
      image={
        <>
          <img
            ...
            alt={performer.name ?? ""}
            src={maybeRenderAltImageA(performer.id) ?? ""}
          />
        </>
      }
      overlays={
