...
// Export maybeRenderAltImageB function
export function maybeRenderAltImageB(performerId: string): string | null {
  const { data } = GQL.useFindImagesQuery({
    variables: {
      image_filter: {
        ...
        tags: {
          modifier: GQL.CriterionModifier.Includes,
          value: ["8"], // insert your alt image tag id # here ex: ["1561"]
        },
      },
    },
  });

  if (data?.findImages.count && data.findImages.count > 0 && data.findImages.images[0]?.paths.image) {
    return data.findImages.images[0].paths.image;
  }
      ...
      image={
        <>
          <img
            ...
            alt={performer.name ?? ""}
            src={maybeRenderAltImageB(performer.id) ?? ""}
          />
        </>
      }
      overlays={
